;;; ixides.el --- List decision with constraints defined by user.
...
;;----------------------------------------------------------------------------;;
(defun ixides-list-read-lines (filePath)
  "Parse text files into list."
  (with-temp-buffer
    (insert-file-contents filePath)
    (setq ixides-germans (split-string (buffer-string) "\n" t))))
